...
bot.onText(/\/help/, (msg, match) => {
  ...
  bot.sendMessage(chatId,'Для того чтобы узнать доходность на сегодня/завтра/вчера наберите в чат /royalty'+'\n');
  bot.sendMessage(chatId,'Для того чтобы расчитать размер своего депозита через выбранное вами количество дней необходимо ввести в чат команду'+'\n'+'/payments <Размер вашего текущего депозита> <кол-во дней> <количество депозитов свыше 1010$>');
...
else 
{
   while (i<parseInt(paydata[2],10)) 
	...
	bot.sendMessage(chatId,'Ваша депозит через '+paydata[2]+' дней составит : '+enddeposit+'$'+'       '+'Ежедневный доход с вашего депозита составит :'+enddeposit*0.0096+'$');
	};
